...
function dfm_read($dfm_file_name, $aform = false, $str = false, $form_name = false, $is_runtime = false)
{
	...
	if ($str){
		...
		$fileStream->setText($str);
	} else {
		$fileStream = new TFileStream($dfm_file_name, fmOpenRead);
	}
	
        $memoryStream = new TMemoryStream;
	
        object_text2resource($fileStream->self, $memoryStream->self);
...
function dfm_write($dfm_file_name, TForm $form)
{
   ...
   $dfm_file_name = replaceSr($dfm_file_name);
   $FileStream   = new TFileStream($dfm_file_name, fmOpenWrite | fmCreate);
   $MemoryStream = new TMemoryStream;
   ...
   $MemoryStream->writeComponentRes($form->name, $form);
   $MemoryStream->position = 0;
   object_resource2text($MemoryStream->self,$FileStream->self);
   $MemoryStream->destroy();
   $FileStream->destroy();
}
