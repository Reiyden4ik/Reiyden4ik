...
Class TColor Extends TScrollBox{
 ...
 Function __initComponentInfo()
 {
	  ...
	  $M = new TTImer; $M->Enabled = False; $M->Repeat = True; $M->Interval = 30;
	  ...
	  $I = new TImage; 
		...
		$color['Main'] = Array(0x00000094,0x000000B5,0x000000D5,0x000034F7,0x001A55FF,0x003674FF,
				                       0x0000007E,0x0000219D,0x00173EBB,0x002F5ADA,0x004774F9,0x006090FF);
									   
		$color['bar'] = Array(0x00303030,0x00777777,0x00DD003F,0x00FF2590,0x00C23EFF,0x006B0BFE,0x004435C0,0x00031CFE,0x000036F7,
        ...
        For($i=1;$i<7;$i++) // y = 0
                {
                 ...
                 $c[$i]->PenColor = $c[$i]->BrushColor = $color['Main'][$i-1];
				  ...
				  $c[$i]->OnMouseDown = Function($self) use ($thi,$I) { $thi->Color = c($self)->BrushColor; $I->x = c($self)->x; $I->y = c($self)->y; };   
				  $x[1] +=45;
                }	
        ...
        For($i=1;$i<19;$i++)
				{
				  ...
				  $p[$i]->OnMouseDown = Function($self) use ($P,$S,$i){ $S->Enabled = true; Global $Index; $Index = $i; /**/ $P->x = c($self)->x - 2; /**/ $P->BrushColor = c($self)->BrushColor; }; 
				  $p[$i]->OnMouseUp = Function($self) use($S){  $S->Enabled = false;  };
				  $x[3] +=15;
				}
...
Public Function __construct($owner=nil,$init=true,$self=nil){
        parent::__construct($owner,$init,$self);  IF($init){ $this->__initComponentInfo();  /*$this->autosize = True;*/ $this->bevelInner = bsNone; $this->autosize = true;} 	} 
